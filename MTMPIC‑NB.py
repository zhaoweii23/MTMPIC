...
class ModelConfig:
    ...
    learning_rates: Dict[str, float] = {
        'base': 5e-5, 'high': 1e-4, 'mid': 5e-5, 'low': 1e-5, 'tiny': 5e-6
    }
    batch_sizes: Dict[str, int] = {
        'high_volume': 32, 'mid_volume': 16, 'low_volume': 8, 'tiny_volume': 4
    }
    ...
    save_dir: str = "experiment_results_NB/saved_models"
    log_dir: str = "experiment_results_NB/logs"
    plot_dir: str = "experiment_results_NB/plots"
...
class CSVSubsetDataset(Dataset):
    def __init__(self, csv_path: str, cache_root: str, group: str, df_with_group: Optional[pd.DataFrame] = None):
...
def create_grouped_datasets(csv_path: str, cache_root: str) -> Dict[str, CSVSubsetDataset]:
    groups = ["high_volume", "mid_volume", "low_volume", "tiny_volume"]
    datasets = {}
    for g in groups:
        datasets[g] = CSVSubsetDataset(csv_path, cache_root, group=g)
    return datasets
...
class GroupedMTLModel(nn.Module):
    def __init__(self, task_groups: Dict[str, Dict[str, Any]]):
        ...
        for group, config in task_groups.items():
            if group == 'high_volume':
                self.group_layers[group] = nn.Identity()
            elif group == 'mid_volume':
                self.group_layers[group] = nn.Sequential(
            ...
            for task in config['tasks']:
                self.heads[task] = nn.Sequential(
                    ...
                    nn.GELU(), nn.Dropout(ModelConfig.dropout_rate),
    ...
    def forward(self, mol_data, prot_data, task, group):
        with torch.amp.autocast(DEVICE_TYPE, enabled=ModelConfig.use_amp):
            ...
            group_feat = self.group_layers[group](joint_feat)
            output = self.heads[task](group_feat)
        return output
...
class GroupedTrainer:
    def __init__(self, model: GroupedMTLModel, train_datasets: Dict[str, Dataset], val_datasets: Dict[str, Dataset]):
        ...
        group_keywords = {
            'high_volume': ['mol_encoder', 'prot_encoder', 'fusion', 'group_layers.high_volume'],
            'mid_volume': ['group_layers.mid_volume'],
            'low_volume': ['group_layers.low_volume'],
            'tiny_volume': ['group_layers.tiny_volume'],
        }

        for group, config in model.task_groups.items():
            for task in config['tasks']:
                group_keywords[group].append(f'heads.{task}')


        group_params = {g: [] for g in group_keywords}
        other_params = []
        for name, param in model.named_parameters():
            assigned = False
            for group, keywords in group_keywords.items():
                if any(kw in name for kw in keywords):
                    group_params[group].append(param)
                    assigned = True
                    break
            if not assigned:
        ...
        for group in ['high_volume', 'mid_volume', 'low_volume', 'tiny_volume']:
            if group_params[group]:
                lr = ModelConfig.learning_rates.get(group, ModelConfig.learning_rates['base'])
                param_groups.append({'params': group_params[group], 'lr': lr})
        if other_params:
            param_groups.append({'params': other_params, 'lr': ModelConfig.learning_rates['base']})

        self.optimizer = optim.AdamW(param_groups, weight_decay=ModelConfig.weight_decay)
        # --------------------------------------------------------
        ...
        self.train_loaders = {}
        for group, ds in train_datasets.items():
            bs = ModelConfig.batch_sizes[group]
            self.train_loaders[group] = DataLoader(
                ds, batch_size=bs, shuffle=True,
                collate_fn=self._collate_fn, num_workers=0
            )
        self.val_loaders = {}
        for group, ds in val_datasets.items():
            bs = ModelConfig.batch_sizes[group]
            self.val_loaders[group] = DataLoader(
    ...
    def _collate_fn(self, batch):
        ...
        return {"mol_feat": mol_feats, "prot_feat": prot_feats, "label": labels, "tasks": tasks, "groups": groups}
    ...
    def train_step(self, batch):
        with torch.amp.autocast(DEVICE_TYPE, enabled=ModelConfig.use_amp):
            ...
            for i in range(len(batch["tasks"])):
                ...
                out = self.model(
                    mol_data=mol_data, prot_data=prot_data,
                    task=batch["tasks"][i], group=batch["groups"][i]
                )
                outputs.append(out)
            outputs = torch.stack(outputs)
            loss = F.cross_entropy(outputs, batch["label"].to(self.device))
            if torch.isnan(loss):
                return None
    ...
    def train_epoch(self):
        ...
        for group_loader in self.train_loaders.values():
            for batch in tqdm(group_loader, desc="Training"):
                if batch is None:
                    continue
    ...
    def evaluate(self, dataloaders=None):
        if dataloaders is None:
            dataloaders = self.val_loaders
            ...
            for loader in dataloaders:
                for batch in tqdm(loader, desc="Evaluating"):
                    ...
                    for i in range(len(batch["tasks"])):
                        ...
                        out = self.model(
                            mol_data=mol_feats, prot_data=prot_feats,
                            task=batch["tasks"][i], group=batch["groups"][i]
                        )
        ...
        y_pred = torch.stack(all_preds).numpy() if all_preds else np.array([])
    ...
    def train(self):
        ...
        for epoch in range(ModelConfig.epochs):
            ...
            logging.info(
                f"Time: {time.time()-start:.1f}s | Train Loss: {train_loss:.4f} | "
...
def run_single_experiment(scenario: str, seed: int, train_csv: str, test_csv: str) -> Dict[str, float]:
    torch.manual_seed(seed)
    np.random.seed(seed)
    if torch.cuda.is_available():
        device_idx = ModelConfig.device.index if ModelConfig.device.index is not None else 0
        torch.cuda.set_device(device_idx)
        torch.cuda.manual_seed_all(seed)

    train_datasets = create_grouped_datasets(train_csv, FEATURE_CACHE_DIR)
    test_datasets = create_grouped_datasets(test_csv, FEATURE_CACHE_DIR)
    ...
    for g in train_datasets:
        ...
        val_datasets[g] = AttributeSubset(ds, indices[:n_val])
        train_datasets[g] = AttributeSubset(ds, indices[n_val:])

    task_groups = {}
    for g, ds in train_datasets.items():
        tasks = list(ds._scanned_tasks)
        if not tasks:
            tasks = [f"default_{g}"]
        task_groups[g] = {'tasks': tasks, 'class_weights': ds.task_label_counts}
        logging.info(f"Group {g}: {len(tasks)} tasks, {len(ds)} train samples")

    model = GroupedMTLModel(task_groups).to(ModelConfig.device)
    trainer = GroupedTrainer(model, train_datasets, val_datasets)
    trainer.train()

    test_metrics = trainer.evaluate(test_datasets)
    return {
...
def generate_latex_table(results):
    ...
    for key, name in names.items():
        if key in results:
            r = results[key]
            latex.append(
                f"{name} & ${r['balanced_accuracy_mean']:.3f} \\pm {r['balanced_accuracy_std']:.3f}$ & "
...
def main():
    ...
    for scenario in args.scenarios:
        ...
        for seed in args.seeds:
            logging.info(f"Running {scenario} seed {seed} (grouped, no balancing)")
